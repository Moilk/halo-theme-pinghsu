	...
	<div class="pagination clearfix">
		<#if posts.totalPages gt 1>
			<#if posts.hasPrevious()>
				<#if posts.number==1>
					<a class="extend prev" href="${context!}/tags/${tag.slugName!}">
						<i class="fa fa-angle-left"></i>
					</a>
				<#else>
					<a class="extend prev" href="${context!}/tags/${tag.slugName!}/page/${posts.number}">
						<i class="fa fa-angle-left"></i>
					</a>
				</#if>
			</#if>
			<#if (rainbow?size> 0 && rainbow[0] >= 2)>
				<a class="page-number" href="${context!}/tags/${tag.slugName!}/page/1">1</a>
				<#if (rainbow[0]> 2)>
			...
			<#if (rainbow?size> 0 && rainbow[rainbow?size-1] < posts.totalPages)>
					...
					<a class="page-number"
						href="${context!}/tags/${tag.slugName!}/page/${posts.totalPages}">${posts.totalPages}</a>
			</#if>
			<#if posts.hasNext()>
				<a class="extend next" href="${context!}/tags/${tag.slugName!}/page/${posts.number+2}">
